...
//anketa_obrada
function  upisi_Anketu($name,$message,$rating){
    global $conn;
        $upit = "INSERT INTO `anketa` (`id_anketa`, `ime`, `poruka`,`broj_zvezda`)
         VALUES (NULL, :ime,:poruka,:broj_zvezda);";
        $unos = $conn->prepare($upit);
        $unos->bindParam(":ime", $name);
        $unos->bindParam(":poruka", $message);
        $unos->bindParam(":broj_zvezda", $rating);

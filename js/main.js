$(document).ready(function () {
  ...
  function anketaInsert() {
    ...
    Dugme.on('click', function () {
      ...
      Proveriparametar(username, usernameRe, "Username is not correct.");
      ...
      if (selectedRating.val() == undefined) {
        spremno = false;
        $('#review-error').text('You have to select some rating');
      } else {
        podaciForme.push(selectedRating.val());
      }


      if (spremno) {
        let ime = podaciForme[0];
        let opis = podaciForme[1];
        let rating = podaciForme[2];
        let button = true;

        let Send = Object.assign({ ime, opis, rating, button });
        ...
        $.ajax({
          ...
          error: function (xhr) {
            ...
            if (xhr.status == 500) {
              $("#review-confirm").html(`
                  <p class="alert alert-warning">
                  There has been an error with a server.
                  </p>`);
            }
            if (xhr.status == 422) {
              $("#review-confirm").html(`
                  <p class="alert alert-warning">
                 Error with the data processing.
                  </p>`);
            }
          }
        });
  ...
  function contactPoruka() {
    ...
    Dugme.on('click', function () {
      ...
      Proveriparametar(username, usernameRe, "Username must be greater then 4 characters.");
      Proveriparametar(number, numberRe, "The number should look something like this: 0635550100");
      Proveriparametar(contactEmail, emailRe, "Your Email is not correct.");
